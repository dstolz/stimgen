    ...
    methods
        ...
        function messageText = format_gui_error_message_(obj, ME, fallbackText)
            ...
            switch string(ME.identifier)
                ...
                % The stimgen.TORC class and the stimgen.components.TORC patch
                % node raise the same conditions and warrant the same guidance.
                case {"stimgen:TORC:TemporalOrthogonality", "stimgen:components:TORC:TemporalOrthogonality"}
                ...
                case {"stimgen:TORC:RateBelowFundamental", "stimgen:components:TORC:RateBelowFundamental"}
                ...
                case {"stimgen:TORC:BandwidthExceedsNyquist", "stimgen:components:TORC:BandwidthExceedsNyquist"}
                ...
                case {"stimgen:TORC:InvalidComponentList", "stimgen:TORC:EmptyComponentList", ...
                      "stimgen:components:TORC:InvalidComponentList", "stimgen:components:TORC:EmptyComponentList"}
                    messageText = "Enter the ripple components as a list of numbers, such as 4 8 12 16 or 4:4:24.";
                case {"stimgen:TORC:InvalidRate", "stimgen:components:TORC:InvalidRate"}
                    messageText = "Component rates must all be positive. Set the direction of travel with the sign of the ripple density instead.";
                case {"stimgen:TORC:InvalidRateRange", "stimgen:components:TORC:InvalidRateRange"}
                ...
                case "stimgen:Patch:CycleDetected"
                    messageText = [ ...
                        "That connection would create a feedback loop." + newline + ...
                        "Each component renders its whole waveform at once, so a loop has no defined meaning. Remove one of the connections between those nodes." + newline + newline + ...
                        string(ME.message) ...
                    ];
                case "stimgen:Patch:NotModulatable"
                    messageText = [ ...
                        "That parameter cannot be driven by another component." + newline + ...
                        "Only the parameters shown as input ports on the left edge of a node accept a connection." + newline + newline + ...
                        string(ME.message) ...
                    ];
                case {"stimgen:Patch:InvalidNodeLabel", "stimgen:Patch:DuplicateNodeLabel"}
                    messageText = [ ...
                        "That node name cannot be used." + newline + ...
                        "Names become part of each parameter's name, so they must be unique and must start with a letter, containing only letters, digits and underscores." + newline + newline + ...
                        string(ME.message) ...
                    ];
                case "stimgen:Patch:UnknownNode"
                    messageText = "That connection or output refers to a component that is no longer in the patch. Open Edit Graph... to reconnect it.";
                case "stimgen:Patch:NoOutputNode"
                    messageText = "No component is selected as the output. Open Edit Graph... and drag a node's output port to the OUT terminal.";
                case "stimgen:Patch:BadNodeOutput"
                    messageText = [ ...
                        "A component produced an unusable waveform." + newline + ...
                        "This usually means a modulated parameter reached zero or went negative -- check the Depth of the connections into that node." + newline + newline + ...
                        string(ME.message) ...
                    ];
                case "stimgen:Patch:NoEqualizer"
                    messageText = "Calibration Mode is set to Filtered, but the loaded calibration has no equalization filter. Design one in the calibration GUI, or set Calibration Mode to Tone or None.";
                case "stimgen:components:NoiseSource:InvalidBand"
                    messageText = "The noise passband is empty. Low Pass must be above High Pass and below half the sample rate.";
                case "stimgen:components:PulseTrain:WidthExceedsPeriod"
                    messageText = "The pulse is longer than the gap between pulses. Shorten Width or lower Rate.";
                case "stimgen:components:PulseTrain:WidthTooShort"
                    messageText = "The pulse is shorter than one sample at the current sample rate. Increase Width or raise Fs.";
                case "stimgen:components:Sweep:BadFreqRange"
                    messageText = "Start Freq must be below Stop Freq.";
                otherwise
