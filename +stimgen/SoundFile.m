    ...
    methods (Access = protected)
        ...
        function m = propMeta(obj)
            ...
            m.BrowseFiles     = struct('label', 'Sound Files', 'widget', 'button', ...
                ...
                'tooltip', 'Add sound files to this object''s catalog. Files are added, not replaced, and each becomes one entry addressable by File Index.');
            m.UseAllFiles     = struct('label', 'File Selection', 'widget', 'button', ...
                                       'text', 'Use All Files', 'callback', 'select_all', ...
                                       'group', 'Waveform', 'order', 2, ...
                'tooltip', 'Set File Index to every file in the catalog, making one variant per file.');
            m.FileIndex       = struct('label', 'File Index', ...
                                       'group', 'Waveform', 'order', 10, ...
                'tooltip', 'Which catalog entries to play, by position. This is the variant axis: 1:8 makes eight variants. Duration follows the selected file.');
            m.Channel         = struct('label', 'Channel (0 = mix)', ...
                                       'group', 'Waveform', 'order', 20, ...
                'tooltip', 'Channel to take from a multichannel file. 0 mixes all channels to mono.');
            m.CalibrationMode = struct('label', 'Calibration Mode', 'widget', 'dropdown', ...
                                       'items', ["Filtered" "Direct" "None"], ...
                                       'group', 'Level', 'order', 5, ...
                'tooltip', 'Filtered = equalize the whole spectrum with the calibration filter, best for broadband material; Direct = no equalization, level taken at Anchor Freq, preserves the native spectrum; None = normalize only.');
            m.AnchorFrequency = struct('label', 'Anchor Freq (Hz, 0 = auto)', ...
                                       'group', 'Level', 'order', 6, ...
                'tooltip', 'Frequency whose calibration value sets the level in Direct mode. 0 uses the waveform''s spectral centroid. Ignored in the other modes.');
            m.LevelReference  = struct('label', 'Level Reference', 'widget', 'dropdown', ...
                                       'items', ["rms" "peak"], ...
                                       'group', 'Level', 'order', 7, ...
                'tooltip', 'Waveform statistic that Sound Level refers to: rms suits continuous material, peak suits transients with a high crest factor.');
